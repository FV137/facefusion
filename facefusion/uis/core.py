...
def launch() -> None:
    ...
    # Get server configuration from state_manager or use defaults
    server_host = state_manager.get_item('server_host') or '0.0.0.0'
    server_port = state_manager.get_item('server_port') or 7860
    server_share = state_manager.get_item('server_share') or False

    logger.info(translator.get('server_starting').format(host = server_host, port = server_port), __name__)
    ui.launch(server_name = server_host, server_port = server_port, share = server_share)
